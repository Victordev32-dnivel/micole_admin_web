...
export class ComunicadosListadoComponent implements OnInit {
  ...
  private loadSalones(): void {
    ...
    this.http
      .get<{data: Salon[]}>(`https://proy-back-dnivel-44j5.onrender.com/api/salon/colegio/lista/${this.colegioId}`, 
      ...
      .subscribe({
        ...
        error: (error) => {
          ...
          this.loadingSalones = false;
          this.snackBar.open('❌ Error al cargar la lista de salones', 'Cerrar', {
            duration: 3000,
      ...
      .subscribe({
        next: (response) => {
          this.ngZone.run(() => {
            ...
            const salonInfo = this.salones.find(s => s.id === this.salonSeleccionado);
            
            if (Array.isArray(response)) {
              this.comunicados = response.map((anuncio: AnuncioSalon) => ({
                id: anuncio.id,
                nombre: anuncio.nombre, // Para salones ya viene como 'nombre'
                horario: anuncio.horario,
                pdf: anuncio.pdf,
                salon: salonInfo ? {
                  id: salonInfo.id,
        ...
        error: (error) => {
          ...
          this.ngZone.run(() => {
            ...
            if (error.status === 404) {
              ...
              this.snackBar.open('ℹ️ Este salón no tiene anuncios disponibles', 'Cerrar', {
                duration: 3000,
  ...
  openUrl(url: string): void {
    ...
    if (!newWindow) {
      this.snackBar.open('❌ No se pudo abrir el enlace. Verifique su bloqueador de ventanas emergentes', 'Cerrar', {
        duration: 5000,
        panelClass: ['error-snackbar'],
        verticalPosition: 'top',
        horizontalPosition: 'center'
