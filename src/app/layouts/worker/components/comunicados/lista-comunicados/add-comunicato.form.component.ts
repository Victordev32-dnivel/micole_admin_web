...
@Component({
  selector: 'app-modal-anuncio-general',
  standalone: true,
  imports: [
    CommonModule,
    ReactiveFormsModule,
...
export class ModalAnuncioGeneralComponent implements OnInit {
  ...
  constructor(
    ...
    public dialogRef: MatDialogRef<ModalAnuncioGeneralComponent>,
  ...
  async onImageUpload(event: Event): Promise<void> {
    ...
    if (input.files && input.files.length > 0) {
      ...
      try {
        const signedUrl = await this.uploadImageToS3(file);
  ...
  removeImage(): void {
    this.imageFile = null;
    this.generalForm.patchValue({ imagen: '' });
    this.uploadProgress = 0;
  }
