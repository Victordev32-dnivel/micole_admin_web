      ...
      <form *ngIf="data.alumnos && data.alumnos.length > 0" [formGroup]="tarjetaForm" class="tarjeta-form">
            ...
            <mat-option 
              *ngFor="let alumno of filteredAlumnos | async" 
              ...
              <div class="alumno-info">
                <div class="alumno-nombre">{{ cleanName(alumno.nombre_completo) }}</div>
                <div class="alumno-details">
                  <span class="alumno-codigo">Código: {{ alumno.codigo || 'Sin código' }}</span>
                  <span class="alumno-documento">DNI: {{ alumno.numero_documento }}</span>
                </div>
              </div>
            </mat-option>
            <mat-option 
              *ngIf="(filteredAlumnos | async)?.length === 0 && tarjetaForm.get('alumnoSearch')?.value" 
              disabled>
              No se encontraron alumnos
            </mat-option>
          </mat-autocomplete>
          <mat-error *ngIf="tarjetaForm.get('alumnoSearch')?.hasError('required')">
        ...
        <div *ngIf="selectedAlumno" class="selected-alumno">
          ...
          <div class="selected-info">
            <div class="selected-name">{{ cleanName(selectedAlumno.nombre_completo) }}</div>
...
export class AddTarjetaModalComponent {
  ...
  constructor(
    ...
    this.filteredAlumnos = this.tarjetaForm.get('alumnoSearch')!.valueChanges.pipe(
      startWith(''),
  ...
  onSearchInput(event: any): void {
    ...
    if (this.selectedAlumno && inputValue !== this.selectedAlumno.nombre_completo) {
      this.clearSelection();
    }
  }
  ...
  onSubmit(): void {
    if (this.isFormValid()) {
      ...
      if (isNaN(tarjetaData.rfid) || tarjetaData.rfid <= 0 || tarjetaData.rfid.toString().length !== 10) {
        alert('RFID debe ser un número de exactamente 10 dígitos');
        return;
      }

      if (isNaN(tarjetaData.idAlumno) || tarjetaData.idAlumno <= 0) {
        alert('Debe seleccionar un alumno válido');
        return;
      }

      if (tarjetaData.codigo.length < 2) {
      ...
      if (!this.selectedAlumno) {
        alert('Debe seleccionar un alumno válido de la lista de opciones');
      }
    }
  }
}