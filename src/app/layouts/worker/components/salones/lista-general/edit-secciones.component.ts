...
export class EditSeccionesComponent implements OnInit {
  ...
  cargarDatos(): void {
    ...
    // Usar el endpoint que funciona para obtener todas las secciones del colegio
    this.http.get<any>(`${this.apiBase}/seccion/colegio/${this.data.idColegio}?page=1`, { headers: this.getHeaders() })
      .subscribe({
        next: (response) => {
          if (response && response.data && Array.isArray(response.data)) {
            const seccion = response.data.find((s: any) => s.id === this.data.id);
        ...
        error: (err) => {
          ...
          this.error = 'Error al cargar los datos de la sección.';
          this.loading = false;
        }
      });
  }
  ...
  guardar(): void {
    ...
    this.http.put(`${this.apiBase}/seccion/${this.data.id}`, datos, { headers: this.getHeaders() })
      .subscribe({
        next: (response) => {
          this.saving = false;
