...
import { UserService } from '../../../../services/UserData'; // Ajusta la ruta según tu estructura
  ...
  template: `
    <div class="main-content">
      <div class="container">
          ...
          <div class="anuncios-container">
            <div *ngFor="let anuncio of anuncios" class="anuncio-card">
              <div class="anuncio-header">
                ...
                <div class="action-buttons">
                  ...
                  >
                    ...
                    >
                      <path
                        d="M12.146.146a.5.5 0 0 1 .708 0l3 3a.5.5 0 0 1 0 .708L14.5 5.207l-3-3L12.854.146a.5.5 0 0 1 .292-.146zM11.207 2.5 13.5 4.793 14.793 3.5 12.5 1.207 11.207 2.5zm1.586 3L10.5 3.207 4 9.707V13h3.293l6.5-6.5z"
                      />
                      <path
                        fill-rule="evenodd"
              ...
              <div class="anuncio-footer">
                <small
                  >Creado: {{ anuncio.fecha_creacion | date : 'short' }}</small
                >
                <small
                  >Actualizado:
                  {{ anuncio.fecha_actualizacion | date : 'short' }}</small
                >
              </div>
            </div>
          </div>
        ...
        <div
          *ngIf="selectedColegioId && !loadingAnuncios && anuncios.length === 0"
      ...
      >
        ...
        <div class="modal-body">
          <div class="delete-confirmation">
            ...
            <p>
              <strong>{{ anuncioToDelete?.titulo }}</strong>
            </p>
      ...
      <div class="modal-container" (click)="$event.stopPropagation()">
        ...
        <div class="modal-body">
          ...
          >
            <div class="form-row">
              <div class="form-group">
                <label for="editNombre"
                  >Nombre del Anuncio <span class="required">*</span></label
                >
                <input
                  type="text"
                  id="editNombre"
              ...
              <div class="form-group">
                <label for="editHorario"
                  >Horario <span class="required">*</span></label
                >
                <input
                  type="text"
                  id="editHorario"
            ...
            <div class="form-row">
              <div class="form-group">
                <label for="editIdColegio"
                  >Colegio <span class="required">*</span></label
                >
                <select
                  id="editIdColegio"
                  name="editIdColegio"
              ...
              <!-- OPCIÓN 2: Mantener el campo pero como solo lectura/informativo -->
              <div class="form-row">
                

                <div class="form-group">
                  <label for="editTipo">Tipo de Anuncio</label>
                  <input
                    type="text"
                    id="editTipo"
                    name="editTipo"
                  ...
                  />
                  <div class="form-help">
                    Los anuncios para socios siempre son de tipo "socio"
                  </div>
                </div>
              </div>
            </div>
            ...
            <div class="form-group">
              ...
              />
              <div class="form-help">
                URL adicional relacionada con el anuncio
              </div>
            </div>

            

            <div class="form-group" *ngIf="editAnuncio.tipo === 'salon'">
              <label for="editIdSalon">ID de Salón</label>
              <input
                type="number"
                id="editIdSalon"
                name="editIdSalon"
                class="form-control"
                [(ngModel)]="editAnuncio.idSalon"
...
export class AnuncioSocioListComponent implements OnInit {
  ...
  loadColegios(): void {
    ...
    this.http
      .get<{ data: Colegio[] }>(`${this.apiUrl}/api/colegio/lista`, { headers })
      .pipe(finalize(() => (this.loadingColegios = false)))
      .subscribe({
        next: (response) => {
  ...
  onSubmitEdit(): void {
    ...
    // ✅ USAR EXACTAMENTE EL MISMO PAYLOAD QUE EN EditarComunicadoComponent
    const updatePayload: any = {
      nombre: this.editAnuncio.nombre?.trim(),
      horario: this.editAnuncio.horario?.trim(),
      idColegio: Number(this.editAnuncio.idColegio) || 1,
      idUsuario: Number(this.editAnuncio.idUsuario) || 2,
      tipo: 'socio', // ✅ SIEMPRE ES 'socio'
    };

    // ✅ AGREGAR CAMPOS OPCIONALES (QUITAR pdf)
    if (this.editAnuncio.imagen && this.editAnuncio.imagen.trim()) {
      updatePayload.imagen = this.editAnuncio.imagen.trim();
    }
    if (this.editAnuncio.url && this.editAnuncio.url.trim()) {
      updatePayload.url = this.editAnuncio.url.trim();
    }
    // ✅ QUITAR pdf - NO SE INCLUYE
    if (this.editAnuncio.idSalon) {
      updatePayload.idSalon = Number(this.editAnuncio.idSalon);
    }

    // ✅ LIMPIAR CAMPOS VACÍOS COMO EN EL OTRO COMPONENTE
    ...
    this.http
      .put<any>(
        `${this.apiUrl}/api/anuncio/${this.currentEditId}`,
        updatePayload,
        { headers }
      )
      .pipe(finalize(() => (this.updatingEdit = false)))
      .subscribe({
        next: (response) => {
        ...
        error: (error) => {
          ...
          let errorMessage =
            'Error al actualizar el anuncio. Verifique los datos.';
          ...
          if (error.status === 400) {
            ...
            errorMessage = 'No tiene permisos para editar este anuncio';
  ...
  confirmDelete(): void {
    ...
    this.http
      .delete<any>(deleteUrl, options)
      .pipe(finalize(() => (this.deleting = false)))
      .subscribe({
        next: (response: any) => {
          console.log('Respuesta del servidor:', response);
          this.successMessage = `Anuncio "${
            this.anuncioToDelete!.titulo
        ...
        error: (error) => {
          ...
          this.deleteErrorMessage =
            'Error al eliminar el anuncio. Intente nuevamente.';
        },
      });
  }
  ...
  private tryDeleteWithPost(): void {
    ...
    this.http.post<any>(deleteUrl, {}, { headers }).subscribe({
      next: (response) => {
      ...
      error: (error) => {
        ...
        this.deleteErrorMessage =
          'No se pudo eliminar el anuncio. Verifique la conexión.';
      },
    });
  }
